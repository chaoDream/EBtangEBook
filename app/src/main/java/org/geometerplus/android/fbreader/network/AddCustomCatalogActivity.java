...
public class AddCustomCatalogActivity extends Activity {
    ...
    private void openCatalog(Uri uri) {
        startActivity(new Intent(FBReaderIntents.Action.OPEN_NETWORK_CATALOG, uri, this, NetworkLibraryPrimaryActivity.class).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TOP));
        finish();
    }
    ...
    private void setupButton(int id, String resourceKey, View.OnClickListener listener) {
        final Button button = (Button)findViewById(R.id.add_custom_catalog_buttons).findViewById(id);
        button.setText(ZLResource.resource("dialog").getResource("button").getResource(resourceKey).getValue());
