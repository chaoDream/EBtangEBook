...
public class AddCatalogMenuActivity extends MenuActivity {
    ...
    private void addItem(String id, int weight) {
        myInfos.add(new PluginApi.MenuActionInfo(Uri.parse("http://data.fbreader.org/add_catalog/" + id), myResource.getResource(id).getValue(), weight));
    }
    ...
    protected void runItem(final PluginApi.MenuActionInfo info) {
        try {
            startActivity(new Intent(getAction()).addCategory(Intent.CATEGORY_DEFAULT).setData(info.getId()));
