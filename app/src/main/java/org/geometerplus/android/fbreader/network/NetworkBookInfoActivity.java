...
public class NetworkBookInfoActivity extends Activity implements NetworkLibrary.ChangeListener {
        ...
        public void run() {
            ...
            if (myBook == null) {
                ...
                if (uri != null && "litres-book".equals(uri.getScheme())) {
                    try {
                        myBook = OPDSBookItem.create(library, myNetworkContext, library.getLinkByStringId("litres.ru"), uri.toString().replace("litres-book://", "http://"));
                    }catch (ZLNetworkException e) {
    ...
    private void setPairLabelTextFromResource(int id, String resourceKey) {
        ((TextView)findViewById(id).findViewById(R.id.book_info_key)).setText(myResource.getResource(resourceKey).getValue());
    }

    private void setPairLabelTextFromResource(int id, String resourceKey, int param) {
        ((TextView)findViewById(id).findViewById(R.id.book_info_key)).setText(myResource.getResource(resourceKey).getValue(param));
    }
