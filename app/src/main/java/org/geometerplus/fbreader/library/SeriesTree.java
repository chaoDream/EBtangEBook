...
public final class SeriesTree extends FilteredTree {
    ...
    private static Filter filter(org.geometerplus.fbreader.book.Series series, Author author) {
        final Filter f = new Filter.BySeries(series);
        return author != null ? new Filter.And(f, new Filter.ByAuthor(author)) : f;
    }

    SeriesTree(IBookCollection collection, PluginCollection pluginCollection, org.geometerplus.fbreader.book.Series series, Author author) {
        super(collection, pluginCollection, filter(series, author));
        Series = series;
    }

    SeriesTree(LibraryTree parent, org.geometerplus.fbreader.book.Series series, Author author, int position) {
