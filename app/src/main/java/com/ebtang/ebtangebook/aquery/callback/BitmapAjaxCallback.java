...
package com.ebtang.ebtangebook.aquery.callback;
...
import com.ebtang.ebtangebook.aquery.AQuery;
import com.ebtang.ebtangebook.aquery.util.AQUtility;
import com.ebtang.ebtangebook.aquery.util.BitmapCache;
import com.ebtang.ebtangebook.aquery.util.Common;
import com.ebtang.ebtangebook.aquery.util.RatioDrawable;
...
public class BitmapAjaxCallback extends AbstractAjaxCallback<Bitmap, BitmapAjaxCallback>{
	...
	public static void async(Activity act, Context context, ImageView iv, String url, Object progress, ImageOptions options, HttpHost proxy, String networkUrl){
	
		async(act, context, iv, url, options.memCache, options.fileCache, options.targetWidth, options.fallback, options.preset, options.animation, options.ratio, options.anchor, progress, options.policy, options.round, proxy, networkUrl);
	...
	public static void async(Activity act, Context context, ImageView iv, String url, boolean memCache, boolean fileCache, int targetWidth, int fallbackId, Bitmap preset, int animation, float ratio, float anchor, Object progress,int policy, int round, HttpHost proxy, String networkUrl){
		...
		if(bm != null){
			...
			cb.url(url).imageView(iv).memCache(memCache).fileCache(fileCache).targetWidth(targetWidth).fallback(fallbackId).preset(preset).animation(animation).ratio(ratio).anchor(anchor).progress(progress).policy(policy).round(round).networkUrl(networkUrl);
			if(proxy != null){
